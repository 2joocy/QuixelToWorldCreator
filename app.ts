                    ...
                    if (albedoThumb && normalThumb && displacementThumb)
                        if (albedo) {
                            ...
                            await sharp(`${quixelLib}\\${dirName}\\${albedo}`)
                                .resize(128, 128)
                                .toFile(`${wcDir}\\${dirName}\\${albedoThumb[0]}_thumb.${albedoThumb[1]}`);
                        }
                    if (normal) {
                        fs.copyFileSync(`${quixelLib}\\${dirName}\\${normal}`, `${wcDir}\\${dirName}\\${normal}`);
                        await sharp(`${quixelLib}\\${dirName}\\${albedo}`)
                            .resize(128, 128)
                            .toFile(`${wcDir}\\${dirName}\\${normalThumb[0]}_thumb.${normalThumb[1]}`);
                    }
                    if (displacement) {
                        fs.copyFileSync(
                            `${quixelLib}\\${dirName}\\${displacement}`,
                            `${wcDir}\\${dirName}\\${displacement}`
                        );
                        await sharp(`${quixelLib}\\${dirName}\\${albedo}`)
                            .resize(128, 128)
